...
class LMDBCursor : public Cursor {
 public:
  ...
  // virtual bool Get(const string &key, string &out_value) 
  // {
  //   MDB_val t_key;
  //   t_key.mv_size=key.size();
  //   t_key.mv_data= const_cast<char*>(key.data());
  //   int mdb_status = mdb_cursor_get(mdb_cursor_, &t_key, &mdb_value_, MDB_SET_KEY);
...
class LMDB : public DB {
 public:
  ...
  virtual void Retrieve(const string& key, string &value)
  {
    LOG(INFO)<<"lmdb not implemented";
  }
  virtual void UpdateValue(const string&key, const string &value)
  {
    LOG(INFO)<<"lmdb not implemented";
  }
